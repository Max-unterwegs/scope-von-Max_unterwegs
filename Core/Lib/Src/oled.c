...
void OLED_SendCmd(uint8_t cmd) {
  ...
  sendBuffer[0] = cmd;
  HAL_SPI_Transmit(&hspi2, sendBuffer, 1, 1000);
  OLED_DC_Set();
}
...
void OLED_Init() {
 ...
 OLED_RES_Clr();
    ...
    OLED_SendCmd(0xa8 ); // Set Multiplex Ration
    OLED_SendCmd(0x7f );
    OLED_SendCmd(0xd5 ); // Set Frame Frequency
    OLED_SendCmd(0x50 ); // 104Hz
  if(USE_HORIZONTAL==180)
    ...
    OLED_SendCmd(0x00 );
    OLED_SendCmd(0xdc ); // Set Display Start Line
    OLED_SendCmd(0x00 );
    OLED_SendCmd(0x20 ); // Set Page Addressing Mode
    OLED_SendCmd(0x81 ); // Set Contrast Control
    OLED_SendCmd(0xdf );
    OLED_SendCmd(0xa4 ); // Set Entire Display OFF/ON
    OLED_SendCmd(0xa6 ); // Set Normal/Reverse Display
    OLED_SendCmd(0xad ); // Set External VPP
    OLED_SendCmd(0x8a );
    OLED_SendCmd(0xd9 ); // Set Phase Leghth
    OLED_SendCmd(0x22 );
    OLED_SendCmd(0xdb ); // Set Vcomh voltage
    OLED_SendCmd(0x35 );
    OLED_NewFrame();
